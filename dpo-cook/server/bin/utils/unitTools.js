...
function playToUnitType(playUnit) {
    const unit = _playUnits[playUnit];
    if (!unit) {
        throw new Error(`can't convert from play unit: '${playUnit}'`);
    }
    return unit;
}
exports.playToUnitType = playToUnitType;
function convertUnits(value, fromUnit, toUnit) {
    const scaleFactor = _unitScaleFactor[fromUnit][toUnit];
    if (!isFinite(scaleFactor)) {
        throw new Error(`can't convert units from '${fromUnit}' to '${toUnit}'`);
    }
