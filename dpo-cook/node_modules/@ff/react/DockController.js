...
const Controller_1 = require("@ff/core/Controller");
class DockController extends Controller_1.default {
    constructor(commander, initialLayout) {
        super(commander);
    ...
    resize(sectionId, index, firstSize, secondSize) {
        const prevState = this.state;
        const nextState = this.state = clone_1.default(prevState);
    ...
    insertPane(anchorId, location, title, closable, componentId) {
        const prevState = this.state;
        const nextState = this.state = clone_1.default(prevState);
    ...
    removePane(paneId) {
        const prevState = this.state;
        const state = this.state = clone_1.default(prevState);
    ...
    movePane(paneId, anchorId, location) {
        const prevState = this.state;
        let nextState = clone_1.default(prevState);
        const panePath = this._getPath(paneId, nextState);
        if (!panePath) {
    ...
    _insertPane(path, location, pane) {
        const stackLayout = path[0].layout;
        const stackIndex = path[0].index;
        const splitLayout = path[1] && path[1].layout;
...
exports.default = DockController;
DockController.defaultLayout = {
    id: "h4NEk34jJQ",
    type: "split",
